...
var page_data = {};
function showPage(web_page, headers, response) {
  var isHtml = /\.html$/.test(web_page);
  headers['content-type'] = isHtml ? 'text/html' : 'text/plain';
  if (page_data[web_page] != null) {
    response.writeHead(200, headers);
    response.end(page_data[web_page]);
  } else {
    require('fs').readFile(web_page, 'utf8', function(err, data) {
      if (err) {
        console.error(err);
        response.writeHead(500, headers);
        response.end();
      } else {
        page_data[web_page] = data;
        showPage(web_page, headers, response); // Recursive call, but since data is a string, the recursion will end
      }
    });
  }
}
...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    helpFile: __dirname + '/views/www/help.html',
    webPage: __dirname + '/views/www/index.html',
  };
  ...
  return function(req, res) {
    ...
    var location = parseURL(req.url.slice(6) || req.url.slice(1));
    ...
    if (!location) {
      ...
      showPage(corsAnywhere.webPage, cors_headers, res);
      return;
    }
    ...
    if (location.host === 'mojang') {
      res.writeHead(200, {'Content-Type': 'text/plain'});
      res.end('fuck you!');
      return;
    }
	
    if (location.host === 'guide' || location.host === 'help') {
	  res.writeHead(200, {'Content-Type': 'text/plain'});
    ...
    if (!hasRequiredHeaders(req.headers)) {
      ...
      return;
		...
		/*var $url = proxyBaseUrl + "/" + req.url;
		var url = $url.split('//');
		var protocol = url[0] + "//";
		var host = url[1].split( '/' )[0];
		url = protocol + host;
		let targetUrl = $url.split(url)[1].slice(1);*/
    ...
    if (corsAnywhere.redirectSameOrigin && origin && location.href[origin.length] === '/' &&
        location.href.lastIndexOf(origin, 0) === 0 || corsAnywhere.redirectSameOrigin && origin && location.href[origin.length] === '/?url=' &&
